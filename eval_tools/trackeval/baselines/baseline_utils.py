...
def mask_NMS(t_data, iou_threshold=0.5, already_sorted=False):
    ...
    # (those overlapping greater than iou_threshold with another mask that has a higher score)
    num_dets = len(t_data['mask_rles'])
    to_remove = [False for _ in range(num_dets)]
    for i in range(num_dets):
        if not to_remove[i]:
            for j in range(i + 1, num_dets):
                if mask_ious_all[i, j] > iou_threshold:
                    to_remove[j] = True
